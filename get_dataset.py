...
from utils import split_last_n, MyIndexer, get_all_seqs
import torch

df = pd.read_parquet("zvuk-interactions.parquet")
df['timestamp'] = df['datetime'].astype('int64')
print(df.columns)
filtered_data = preprocessing(df, users_sample=None, item_min_count=101, min_len=10, core=False, encoding=False,
                  drop_repeats=False, user_id='user_id', item_id='track_id', timestamp='timestamp',
                  path_to_save=None) 
filtered_data = filtered_data.rename(columns={"user_id": "user_idx", "item_id": "item_idx"})
...
print("test", test.user_idx.nunique(), test.user_idx.max(), test.user_idx.min(), len(test))
